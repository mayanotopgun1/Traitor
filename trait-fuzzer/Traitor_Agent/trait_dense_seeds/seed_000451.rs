...
trait SliceExt<'input> {
    fn new(slice: &'input [u8], extra: u32) -> Self where Self: Sized;
}
...
#[no_mangle]
pub fn extra(s: &[u8]) {
    let _slice = ExtraSlice::new(s, s.len() as u32);
}

struct Zst;

trait SliceExtZst<'input> {
    fn new(slice: &'input [u8], extra: Zst) -> Self where Self: Sized;
}
