#![allow(unused_doc_comments)]
#![feature(auto_traits)]
#![feature(negative_impls)]

auto trait Auto {}
unsafe auto trait AutoUnsafe {}

impl !Auto for bool {}
impl !AutoUnsafe for bool {}

struct AutoBool(#[allow(dead_code)] bool);

impl Auto for AutoBool {}
unsafe impl AutoUnsafe for AutoBool {}

fn take_auto<T: Auto>(_: T) {}
fn take_auto_unsafe<T: AutoUnsafe>(_: T) {}

trait AutoExt: Auto {
    fn auto_ext_method(&self) {}
}
impl<T: Auto> AutoExt for T {}

trait AutoUnsafeExt: AutoUnsafe {
    unsafe fn auto_unsafe_ext_method(&self) {}
}
impl<T: AutoUnsafe> AutoUnsafeExt for T {} // Removed `unsafe` from here

fn main() {

    auto trait AutoInner {}
    unsafe auto trait AutoUnsafeInner {}

    take_auto(0);
    take_auto(AutoBool(true));
    take_auto_unsafe(0);
    take_auto_unsafe(AutoBool(true));

    let _: &(dyn Send + Auto) = &0;
}