...
trait ConstProvider {
    const CONST: &'static dyn MetaTrait;
}

impl<A: 'static> ConstProvider for Foo<A> {
...
trait ConstExt: Bar {
    fn get_const_value(&self) -> i32 { self.get_const().get_value() }
}

impl<T: ?Sized + Bar> ConstExt for T {}

fn main() {
    let foo = Foo::<i32> { _value: 10 };
    let bar: &dyn Bar = &foo;
    println!("const {}", bar.get_const_value());
}