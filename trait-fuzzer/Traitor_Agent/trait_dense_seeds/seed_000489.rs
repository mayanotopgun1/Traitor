...
pub trait Field {
    fn field(&self, _: impl Sized) -> Result<(), std::fmt::Error> {
...
pub trait Dt {
    fn debug_tuple(&self, _: &str) -> fmt::DebugStruct {
        panic!("got into debug_tuple");
    }
}

trait FieldExt: Field {
    fn field_twice(&self, x: impl Sized, y: impl Sized) -> Result<(), std::fmt::Error> {
        self.field(x)?;
        self.field(y)
    }
}
...
trait DtExt: Dt {
    fn debug_tuple_twice(&self, x: &str, y: &str) -> fmt::DebugStruct {
        let _ = self.debug_tuple(x);
        self.debug_tuple(y)
    }
}
