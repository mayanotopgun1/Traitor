...
trait FooTrait {
    fn pin_ref(self: Pin<&Self>) -> Pin<&Self>;
    fn pin_mut(self: Pin<&mut Self>) -> Pin<&mut Self>;
    fn pin_pin_pin_ref(self: Pin<Pin<Pin<&Self>>>) -> Pin<Pin<Pin<&Self>>>;
    fn pin_ref_impl_trait(self: Pin<&Self>) -> impl Clone + '_;
}
...
impl<T: Unpin, U: Unpin> BarTrait<T, U> for Bar<T, U> {
    fn fields(self: Pin<&mut Self>) -> (Pin<&mut T>, Pin<&mut U>) {
