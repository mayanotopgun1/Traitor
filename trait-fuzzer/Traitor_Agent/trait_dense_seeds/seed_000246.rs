...
default impl<T> Search for T {
    fn search(_: (bool, bool, bool)) -> u32 {
        unreachable!()
    }
}
...
impl Search for () {
    fn search(target: (bool, bool, bool)) -> u32 {
        ...
        match x {
            ((a, _) | (_, a), (b @ _, _) | (_, b), (c @ false, _) | (_, c))
                if {
                    guard_count += 1;
                    (a, b, c) == target
                } =>
...
default impl<T> SearchOldStyle for T {
    fn search_old_style(_: (bool, bool, bool)) -> u32 {
        unreachable!()
    }
}
...
default impl<T> SearchWithDummy for T {
    fn search_with_dummy(_: (bool, bool)) -> u32 {
        unreachable!()
    }
}
