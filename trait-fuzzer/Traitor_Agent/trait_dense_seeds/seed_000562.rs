...
impl NoFoo {
    fn new(x: isize, y: isize) -> NoFoo {
        NoFoo { copied: x, nocopy: ncint::new(y) }
    }
}
...
impl NoFooExt for NoFoo {
    ...
    fn update_nocopy(&mut self, value: isize) {
        self.nocopy = ncint::new(value);
    }
}

struct MoveFoo { copied: isize, moved: Box<isize> }
impl MoveFoo {
    fn new(x: isize, y: isize) -> MoveFoo {
        MoveFoo { copied: x, moved: Box::new(y) }
    }
}
...
impl DropNoFoo {
    fn new(x: isize, y: isize) -> DropNoFoo {
        DropNoFoo { inner: NoFoo::new(x, y) }
    }
}
impl Drop for DropNoFoo {
    fn drop(&mut self) {}
}

struct DropMoveFoo { inner: MoveFoo }
impl DropMoveFoo {
    fn new(x: isize, y: isize) -> DropMoveFoo {
        DropMoveFoo { inner: MoveFoo::new(x, y) }
    }
}
impl Drop for DropMoveFoo {
    fn drop(&mut self) {}
}

fn test0() {
    let mut f = DropNoFoo::new(1, 2);
    let b = DropNoFoo { inner: NoFoo { nocopy: ncint::new(3), ..f.inner }};
    let c = DropNoFoo { inner: NoFoo { nocopy: ncint::new(4), ..f.inner }};
...
fn test2() {
    ...
    let b = NoFoo { nocopy: ncint::new(23), ..f };
