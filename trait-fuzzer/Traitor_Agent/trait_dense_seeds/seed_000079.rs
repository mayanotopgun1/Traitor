...
pub trait MatrixJsBufferView<R>: JsBufferView
where
    <DefaultAllocator as Allocator<R>>::Buffer: JsBufferView,
{
    fn get_js_buffer_view(&self) -> &Self;
}

impl<R> MatrixJsBufferView<R> for MatrixMN<R>
where
    DefaultAllocator: Allocator<R>,
    <DefaultAllocator as Allocator<R>>::Buffer: JsBufferView,
{
...
impl<R, S> JsBufferView for Matrix<R, S>
where
    S: JsBufferView,
{
    fn js_buffer_view(&self) -> usize {
        self.data.js_buffer_view()
    }
}