...
trait ArrayWindowsExampleExt<T: Clone> {
    fn array_windows_example_ext<const N: usize>(&self) -> ArrayWindowsExample<'_, T, N>;
}

impl <T: Clone> ArrayWindowsExampleExt<T> for [T] {
    fn array_windows_example_ext<const N: usize>(&self) -> ArrayWindowsExample<'_, T, N> {
        self.array_windows_example::<N>()
    }
}
...
fn main() {
    ...
    for array in v.as_slice().array_windows_example_ext::<FOUR>() {
